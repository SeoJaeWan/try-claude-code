...
Use this whenever `plan-materialize` creates, updates, deletes, or skips source-tree tests. This file defines common source-tree test authoring mechanics; use the active review wiki for policy-level rules about test ownership, final recipients, behavior-example names, and selected coverage.
...
- Put plan-clause tracing, phase names, test type, coverage tables, and materialization mechanics in `materialize.md`, not in source-tree test names.
- Do not name tests after internal materialization terms such as `coverage`, `matrix`, `boundary`, `contract`, `owner`, `final recipient`, or `surface` unless those are exact product/domain terms in the repository.
