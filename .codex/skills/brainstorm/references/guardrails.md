...
- Do not turn test strategy locking into source-tree test design; lock only the goal, verification unit, observable result, stable identifier policy, and excluded scope needed for deterministic planning.
...
- If test strategy choices would change the plan, lock `unit`, `Component Test`, or `E2E` responsibility before handoff unless the user explicitly defers that choice.
