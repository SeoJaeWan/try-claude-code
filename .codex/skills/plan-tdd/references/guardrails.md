...
- Do not treat a missing implementation tree, runner config, or harness as a writing blocker when the plan explicitly locks the first-time runner, command, spec root, source/test topology, and scenario contract.
- Do not treat plan-locked future source/test topology as speculative just because implementation files do not exist yet.
...
- Do not rely on negative/no-op assertions alone when the plan also defines a valid output; author the positive path first
...
- Do not call a red contract valid when the failure is caused by malformed test code, an unplanned locator, an unplanned import path, or an unplanned setup mismatch.
...
- Do not leave English planner shorthand in `tdd.md` explanations or test intent strings unless it is an exact identifier, schema key, command, runner term, or quoted plan text
- Apply `references/test-authoring-conventions.md` so source-tree test intent is readable without opening `tdd.md`.
