...
3. Re-locate the selected text.
   - Resolve `source_path` relative to the plan wiki root.
...
4. Classify the requested change.
   ...
   - Reject only when the requested change contradicts raw evidence, repo-local truth, or the plan wiki source model.
...
- Do not route docs feedback through `plan-wiki-ingest`; feedback is not review evidence.
