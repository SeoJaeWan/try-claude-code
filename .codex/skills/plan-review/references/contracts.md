...
Provide an independent, read-only review of an executable plan file before execution. The review checks whether the plan can be executed without hidden decisions, whether it follows the active plan wiki plan artifact contract, and whether later verification can derive tests or commands without guessing.
...
8. Directly referenced local prerequisite plans only for one-hop prerequisite parity.
9. Controller-verified Figma inventory manifest and snapshot artifacts only when the plan depends on Figma inventory, classification, or plan-local `figma-contract` artifacts.
