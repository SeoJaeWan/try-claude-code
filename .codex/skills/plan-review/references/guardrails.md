...
- Do not edit the plan, source code, source-tree tests, review wiki, or local helper artifacts.
...
- Do not bypass the resolved `review_wiki_root` with hardcoded external paths after the workspace sync path is available.
- In orchestrated mode, do not redo review wiki bootstrap already owned by the orchestrator.
- In orchestrated mode, do not invent alternate `plan_path`, `task_slug`, `review_wiki_root`, or `plan_signature` metadata.
