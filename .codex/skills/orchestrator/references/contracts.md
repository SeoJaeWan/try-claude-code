...
8. `../figma-inventory-snapshot/SKILL.md` only when a controller-verified Figma inventory artifact is already required by the locked planning input.
...
- For architect and reviewer passes, the first bounded wait should normally be at least 3 minutes, and 5 minutes is preferred when the workflow is otherwise blocked on that pass.
