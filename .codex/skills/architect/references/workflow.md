...
- Re-check that every evidence row points under `evidence/**`, matches the plan's phase/input/output/state/function/recipient contract, is clearly planning-only, and gives the browser reviewer the concrete judgment material it claims to provide. For UI scope, re-check that shell/screen/component/state evidence is separated when those are separate implementation units. For function or adapter scope, re-check that input, function/adapter, output recipient, and negative/no-op examples are visible.
