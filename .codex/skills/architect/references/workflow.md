...
- In orchestrated mode:
  - treat provided `task_slug`, `plan_path`, and `plan_wiki_root` as authoritative.
...
- Apply the active plan wiki plan artifact contract for:
  - required sections.
...
- Emit multiple standalone plans only when the active plan wiki contract supports independently mergeable, independently reviewable, rollback-safe boundaries.
...
- Do not create extra navigation or graph artifacts outside the active plan wiki plan artifact contract.
...
- If Figma URL parity is acceptance, route comparison to the appropriate Figma parity audit phase under the active plan wiki contract.
