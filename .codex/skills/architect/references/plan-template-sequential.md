...
- [ ] 단계 상세 문서의 행동 변경 시나리오에 stable `scenario_id`, 검증 단위, 관찰 지점, 식별자 정책, 필요한 경우 E2E 선택 이유가 보인다.
- [ ] 테스트 환경이 아직 없는데 구현 전 TDD가 필요하면 runner, command, spec root, test-owner placement, mock/API fixture 정책, expected red reason이 잠겨 있다.
