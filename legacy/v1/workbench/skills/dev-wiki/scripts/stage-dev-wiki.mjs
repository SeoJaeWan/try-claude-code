...
async function upsertWorkspaceIndex(filePath, workspaceRoot, project) {
  const existing = (await readJsonIfExists(filePath)) || { workspaces: {} };
  const workspaces = existing.workspaces && typeof existing.workspaces === "object" ? existing.workspaces : {};
  const previous = workspaces[workspaceRoot] || {};
  const next = {
    ...previous,
    project,
  ...
  workspaces[workspaceRoot] = next;
  mkdirSync(path.dirname(filePath), { recursive: true });
  await writeFile(filePath, `${JSON.stringify({ workspaces }, null, 2)}\n`, "utf8");
  return true;
}
...
async function main() {
  ...
  const devWikiRoot = fullPath(takeFlag("--dev-wiki-root") || defaultDevWikiRoot());
  const configPath = path.join(devWikiRoot, "config.json");
  const workspaceIndexPath = path.join(devWikiRoot, "workspaces.json");
  const existingConfig = (await readJsonIfExists(configPath)) || {};
  const existingWorkspaceIndex = (await readJsonIfExists(workspaceIndexPath)) || { workspaces: {} };
  const workspaceKey = realpathSync(workspaceRoot);
  const existingWorkspace = existingWorkspaceIndex.workspaces?.[workspaceKey] || {};
  ...
  writeTextIfMissing(
    ...
    [
      "# Workbench Dev Wiki",
      "",
      "Workbench-owned dev wiki data lives here instead of inside each project workspace.",
      "",
      "- Config: `config.json`",
      "- Workspace mapping: `workspaces.json`",
      "- Source clone: `source`",
      "- Project wiki root: `source/{project}`",
      ""
    ].join("\n")
  );
