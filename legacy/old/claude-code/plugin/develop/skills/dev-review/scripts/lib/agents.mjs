 ...
 *
 * - 워크스페이스 로컬 위치(`claude-plugin/develop/agents`, `.claude/agents`).
 *   `claude-plugin/develop/agents` 항목은 본 제너레이터가 플러그인 자체 소스 트리
 ...
 *   가리킨다(이 플러그인의 경우 소스 트리에선 `claude-plugin/develop/`, 설치 환경
...
export function defaultAgentsDirs(workspaceRoot) {
  const dirs = [
    path.resolve(workspaceRoot, "claude-plugin/develop/agents"),
