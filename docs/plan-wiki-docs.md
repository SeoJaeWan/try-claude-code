...
Plan Wiki Docs는 `~/.codex/planWiki`를 그대로 읽어 문서 사이트처럼 보여주는 로컬 서버다. 별도 docs 원본을 만들지 않고, 기존 `wiki/core/**`, `wiki/patterns/**`, `wiki/tags/**`, `raw/**`, `history/**`를 화면에 렌더링한다.
...
```powershell
npm run plan-wiki:docs -- --wiki-root C:\Users\USER\.codex\planWiki --port 9788
```
