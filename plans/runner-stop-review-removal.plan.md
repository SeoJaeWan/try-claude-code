...
| 완료 기준 | (a) hooks.json에 Stop 훅 없음, (b) `rg "stop[_-]review\|STOP_REVIEW\|Codex" claude-plugin/develop/scripts/` 결과가 lib/codex 잔재 0건, (c) state.json 스키마가 7개 필드(plan_slug, plan_path, owner_agent, task_branch, worktree_path, base_branch, dev_review) 이하, (d) dev-review skill을 한 줄도 수정하지 않음, (e) 모든 unit test가 통과하며 deprecated 케이스는 같이 제거됨. |
...
| --- | --- |
| `owner_agent` | `general-developer` |
| 변경 경계 | `claude-plugin/develop/hooks/hooks.json`, `claude-plugin/develop/scripts/` 전반 (lib + entry + tests), `claude-plugin/develop/prompts/`, `claude-plugin/develop/skills/runner/SKILL.md`, `claude-plugin/develop/skills/runner/references/`, `claude-plugin/develop/skills/runner/evals/evals.json`, `docs/claude-code-workflow-evolution.md`, repository-level `README.md` (해당되는 경우), `package.json` (test script 정리) |
| 유지 경계 | `claude-plugin/develop/skills/dev-review/` 전체, `claude-plugin/develop/skills/runner/references/prompts/plan-dispatch.md` 및 `rework-dispatch.md` (rework은 stop-review와 무관), `claude-plugin/develop/agents/`, `claude-plugin/develop/skills/commit/`, `claude-plugin/develop/skills/pr/`, plugin manifest 외 다른 skill들, `claude-plugin/statusline/` (별개 플러그인) |
...
| --- | --- | --- |
| `claude-plugin/develop/hooks/hooks.json:37-47` | `Stop` 이벤트 등록 — `stop-review-gate-hook.mjs`, timeout 960s | Phase 1에서 Stop 블록 삭제 |
| `claude-plugin/develop/scripts/stop-review-gate-hook.mjs` | Stop 훅 본체. `codex.mjs`/`diagnostics.mjs`/`prompts.mjs`/`workspace.mjs`/`stop-review-outcome.mjs`/`runner-state.mjs` 7개 모듈 의존 | Phase 1에서 파일 삭제 |
| `claude-plugin/develop/scripts/lib/codex.mjs`, `app-server.mjs`, `app-server-protocol.d.ts`, `process.mjs`, `stop-review-outcome.mjs`, `diagnostics.mjs`, `prompts.mjs`, `workspace.mjs` | grep 결과 모두 stop-review-gate-hook을 직접/간접 통해서만 import됨 (`lib/args.mjs`는 import 0건 — dead code) | Phase 1에서 8개 lib 파일 + dead args.mjs 일괄 삭제 |
| `claude-plugin/develop/prompts/stop-review-gate.md` | Codex 리뷰 프롬프트 템플릿 | Phase 1에서 삭제. `prompts/` 디렉터리 빈 폴더는 git 추적 안 되므로 자연 정리 |
...
| `claude-plugin/develop/scripts/runner-state-fixup.mjs:12,53,72,149` | `--clear-armed`(stop-review)와 `--force-status`(status enum)가 주요 플래그. 두 기능 모두 본 plan에서 제거되는 개념 | Phase 3에서 파일 전체 삭제. 실제 복구 시나리오는 jq/hand-edit 가이드로 대체 |
...
| `claude-plugin/develop/skills/runner/evals/evals.json` | runner skill 평가용 픽스처. stop_review/status 시나리오 포함 추정 | Phase 5에서 stop-review 시나리오 제거, dev-review 중심 케이스만 남김 |
| `claude-plugin/develop/scripts/__tests__/stop-review-gate-hook.test.mjs`, `record-stop-review.test.mjs` | stop-review 전용 테스트 | Phase 6에서 두 파일 삭제 |
| `claude-plugin/develop/scripts/__tests__/runner-state.test.mjs`, `runner-state-cli.test.mjs`, `user-prompt-submit-hook.test.mjs` | status 전이/validateState 단언/`createInitialState` 호출에 stop_review 필드 포함하는 픽스처 다수 | Phase 6에서 status 관련 케이스 삭제, fixture를 슬림 스키마로 갱신 |
...
| --- | --- | --- | --- | --- | --- |
| `claude-plugin/develop/hooks/hooks.json` | `config` | `modify` | `P1` | `Stop` 키 블록 제거. `SessionStart`/`SessionEnd`/`UserPromptSubmit` 그대로 유지. | 위 근거 표 1번 항목. |
| `claude-plugin/develop/scripts/stop-review-gate-hook.mjs` | `source` | `delete` | `P1` | Stop 훅 본체 — 더 이상 호출되지 않음. | 위 근거 표 2번 항목. |
| `claude-plugin/develop/scripts/lib/codex.mjs` | `source` | `delete` | `P1` | stop-review만 사용. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/app-server.mjs` | `source` | `delete` | `P1` | codex.mjs만 사용. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/app-server-protocol.d.ts` | `source` | `delete` | `P1` | app-server.mjs/codex.mjs 타입 정의. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/process.mjs` | `source` | `delete` | `P1` | codex.mjs/app-server.mjs만 사용. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/stop-review-outcome.mjs` | `source` | `delete` | `P1` | codex.mjs와 hook만 사용. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/diagnostics.mjs` | `source` | `delete` | `P1` | `logStopHookEvent` 단일 export — hook만 호출. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/prompts.mjs` | `source` | `delete` | `P1` | `loadPromptTemplate`/`interpolateTemplate` — hook만 호출. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/workspace.mjs` | `source` | `delete` | `P1` | `resolveWorkspaceRoot` — hook만 호출. | 위 근거 표 3번 항목. |
| `claude-plugin/develop/scripts/lib/args.mjs` | `source` | `delete` | `P1` | grep 결과 import 0건 — dead code. | 위 근거 표 3번 항목 끝. |
| `claude-plugin/develop/prompts/stop-review-gate.md` | `docs` | `delete` | `P1` | Codex 리뷰 프롬프트 템플릿. | 위 근거 표 4번 항목. |
...
| `claude-plugin/develop/scripts/lib/runner-state-machine.mjs` | `source` | `delete` | `P2` | 잔존 가치(`DEV_REVIEW_PHASE`) 흡수 후 파일 자체 삭제. | 위 근거 표 6번 항목. |
| `claude-plugin/develop/scripts/runner-state-cli.mjs` | `source` | `modify` | `P3` | `begin-rework`, `rework-done`, `mark-qa-pending`, `qa-resolved`, `reset` 5개 subcommand만. status 검증 호출 모두 제거. | 위 근거 표 7번 항목. |
| `claude-plugin/develop/scripts/runner-state-fixup.mjs` | `source` | `delete` | `P3` | `--clear-armed`(stop-review), `--force-status`(status enum) 모두 본 plan에서 제거됨. | 위 근거 표 8번 항목. |
| `claude-plugin/develop/scripts/user-prompt-submit-hook.mjs` | `source` | `modify` | `P4` | (a) `createInitialState` 새 시그니처에 맞춰 호출 단순화, (b) `TERMINAL_STATUSES` 체크 제거 → `.merged` 마커 파일 체크로 교체, (c) status 관련 prose/주석 정리. `detectBaseBranch`는 유지. | 위 근거 표 9번 항목. |
...
| `claude-plugin/develop/skills/runner/references/enforcement.md` | `docs` | `modify` | `P5` | "Stop hook + dev-review browser UI" 게이트 항목 제거. CLI subcommand 카탈로그 축소된 목록으로 갱신. | 위 근거 표 11번 항목. |
| `claude-plugin/develop/skills/runner/references/dev-review-flow.md` | `docs` | `modify` | `P5` | "Why the Step-3 deadlock matters" 섹션 제거, rework이 `arm-for-dispatch`를 호출하지 않는다는 단락 제거 (이제 arm-for-dispatch 자체가 없음). | 위 근거 표 11번 항목. |
| `claude-plugin/develop/skills/runner/references/plan-state-recovery.md` | `docs` | `modify` | `P5` | `stop_review.*` 필드 행 제거, `--clear-armed`/`--force-status` 레시피 제거, status 흐름 다이어그램에서 `dispatching` 제거, 슬림 스키마 기준으로 jq hand-edit 예시 갱신. | 위 근거 표 11번 항목. |
| `claude-plugin/develop/skills/runner/references/glossary.md` | `docs` | `keep` | `P5` | grep 매치 0건 — stop-review 용어 미수록. | 검증된 사실. |
| `claude-plugin/develop/skills/runner/references/guardrails.md` | `docs` | `keep` | `P5` | grep 매치 0건. | 검증된 사실. |
| `claude-plugin/develop/skills/runner/evals/evals.json` | `data` | `modify` | `P5` | stop-review 시나리오 제거, 디스크 + phase 라우팅 시나리오로 갱신. | 위 근거 표 12번 항목. |
| `docs/claude-code-workflow-evolution.md` | `docs` | `modify` | `P5` | stop-review 언급 정리. | 위 근거 표 13번 항목. |
| `claude-plugin/develop/scripts/__tests__/stop-review-gate-hook.test.mjs` | `test` | `delete` | `P6` | 대상 hook이 사라짐. | 위 근거 표 14번 항목. |
| `claude-plugin/develop/scripts/__tests__/record-stop-review.test.mjs` | `test` | `delete` | `P6` | 대상 CLI subcommand가 사라짐. | 위 근거 표 14번 항목. |
| `claude-plugin/develop/scripts/__tests__/runner-state.test.mjs` | `test` | `modify` | `P6` | status 전이/validateState 단언 케이스 삭제. `createInitialState` 픽스처를 슬림 스키마로 갱신. dev_review phase mutator 케이스는 유지·확장. | 위 근거 표 15번 항목. |
| `claude-plugin/develop/scripts/__tests__/runner-state-cli.test.mjs` | `test` | `modify` | `P6` | 사라진 subcommand 테스트 삭제. 남는 5개 subcommand는 status 가드 없이 동작하므로 케이스 단순화. | 위 근거 표 15번 항목. |
| `claude-plugin/develop/scripts/__tests__/user-prompt-submit-hook.test.mjs` | `test` | `modify` | `P6` | `.merged` 마커 거부 케이스 추가. status/createInitialState 픽스처 슬림 스키마로 갱신. | 위 근거 표 15번 항목. |
...
| dev-review skill 무수정 | command | `git diff main -- claude-plugin/develop/skills/dev-review/` | 빈 결과 |
| 삭제 파일 부재 | command | `ls claude-plugin/develop/scripts/lib/codex.mjs claude-plugin/develop/scripts/stop-review-gate-hook.mjs 2>/dev/null` | 둘 다 부재 |
| 전체 테스트 통과 | command | `node --test claude-plugin/develop/scripts/__tests__/*.test.mjs` | 모두 green |
...
| dev-review skill이 `state.status`/`state.stop_review` 같은 부재 필드를 읽어 NPE | dev-review가 부재 필드를 참조하면 그 호출 자체가 첫 호출에서 깨짐 | 위 변경 경계 표대로 dev-review skill은 본 plan에서 한 줄도 안 건드림 → 사실상 위험 0. 본 plan 적용 후 `git diff main -- claude-plugin/develop/skills/dev-review/`로 검증. |
...
- [ ] 변경 경계와 유지 경계가 분명하다 — 특히 `claude-plugin/develop/skills/dev-review/`는 한 줄도 안 건드림이 명시되어 있다.
