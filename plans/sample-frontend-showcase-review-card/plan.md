...
# sample-frontend-showcase-review-card 실행 계획 (Sequential)

## 요약

- 목표: `features/ui-publish-fixture`의 `/showcase` 리뷰 카드 영역을 `frontend-developer` 단독 샘플로 정리해, Claude와 Codex 모두에서 visual-only 실행 흐름을 검증한다.
...
- 구현 결과는 `frontend-developer`가 단독으로 처리할 수 있는 visual-only 범위로 제한한다.
...
- 목적: ReviewCard visual shell과 showcase section contract를 하나의 visual-only source of truth로 정리해, unified frontend 플로우 검증에 필요한 경계와 locator를 명확히 확보한다.
- 작업:
...
- 산출물 위치:
  - `plans/sample-frontend-showcase-review-card/e2e/manifest.md`
  - `plans/sample-frontend-showcase-review-card/e2e/showcase-review-card.spec.ts`
...
- 신규:
  ...
  - `plans/sample-frontend-showcase-review-card/plan.md`
  - `plans/sample-frontend-showcase-review-card/e2e/manifest.md`
  - `plans/sample-frontend-showcase-review-card/e2e/showcase-review-card.spec.ts`
- 삭제:
...
- [ ] visual scope가 unified `frontend-developer` 범위를 넘지 않음
