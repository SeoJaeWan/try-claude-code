...
export function resolveGenerationContext({ workspaceRoot = REPO_ROOT, devWikiRoot = null, project = null } = {}) {
  const root = path.resolve(workspaceRoot);
  ensureDirectory("Workspace root", root);

  let wikiRoot = path.resolve(devWikiRoot || defaultDevWikiRoot());
  let configPath = path.join(wikiRoot, "config.json");
  ...
  const workspaceIndex = readJsonIfExists(path.join(wikiRoot, "workspaces.json")) || { workspaces: {} };
  const workspaceEntry = workspaceIndex.workspaces?.[workspaceKey] || workspaceIndex.workspaces?.[path.resolve(root)] || {};
  const resolvedProject = project || workspaceEntry.project || config.project || readPackageName(root) || path.basename(root);
  if (!resolvedProject) throw new Error("Dev wiki project is missing in config.");
  const sourceRoot = path.join(wikiRoot, "source");
  ...
  return { workspaceRoot: root, devWikiRoot: wikiRoot, project: resolvedProject, graphRoot };
}
