...
Perform stages 9–11 on the integrated coordinator result, never on isolated worker branches.

Read [references/finalization-report.md](references/finalization-report.md) before testing or reviewing.

## Entry gate

- Require a ready Shape Report, Execution Plan, all Task Results, and `integrated_head_sha` for the same `run_id` and Git common dir.
- Require the current directory to be the recorded linked coordinator worktree, at the exact integrated head, with a clean status.
...
- Produce a concise Markdown report using the reference schema: summary, decisions, changes, verification, independent findings, limitations, task commits, integrated head, and handoff state.
...
Do NOT merge/rebase into Local, push, publish, open a PR, hand off the task, delete worktrees/branches, clean user changes, or invoke another Workbench skill automatically. Report Local branch drift and let the user choose the delivery action.
