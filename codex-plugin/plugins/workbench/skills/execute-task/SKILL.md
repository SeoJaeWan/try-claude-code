...
Use GPT-5.6 Sol with `high` reasoning effort as the recommended coordinator. Select it in the calling task; this skill does not switch the current task's model. Implementation and integration workers use the fixed Luna/xhigh profile below.
...
5. Spawn one worker per runnable task with no conversation history, the complete normalized runtime packet, model `gpt-5.6-luna`, and reasoning effort `xhigh`. Do not silently fall back to another model or effort.
