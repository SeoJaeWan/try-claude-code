...
export default function LoginPage() {
  const { form, handleChange, handleBlur, validate, setErrors } = useLoginForm();
  const { login, error: generalError } = useLogin();
    ...
    <div className="mx-auto mt-20 max-w-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          />
          {form.errors.email && <p className="mt-1 text-xs text-red-600" data-testid="email-error">{form.errors.email}</p>}
        </div>
        ...
        <div>
          ...
          />
          {form.errors.password && <p className="mt-1 text-xs text-red-600" data-testid="password-error">{form.errors.password}</p>}
        </div>
