...
export default function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col border-r border-zinc-200 bg-white transition-transform duration-200 dark:border-zinc-800 dark:bg-zinc-900 md:static md:translate-x-0 md:w-60 lg:w-64 ${
      ...
      >
        ...
        <div className="flex h-14 items-center justify-between border-b border-zinc-200 px-5 dark:border-zinc-800">
          ...
          <button
            type="button"
            onClick={() => setSidebarOpen(false)}
            className="rounded-lg p-1 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800 md:hidden"
            aria-label="사이드바 닫기"
          >
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
        ...
        {/* Navigation links */}
        <nav className="flex flex-1 flex-col gap-1 px-3 py-4" aria-label="메인 내비게이션">
          {navLinks.map((link) => {
        ...
        <div className="border-t border-zinc-200 px-3 py-4 dark:border-zinc-800">
          <button
            type="button"
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-200"
