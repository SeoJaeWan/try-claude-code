...
// 각 테스트마다 고유한 이메일 사용 (병렬 실행 시 충돌 방지)
function makeTestUser() {
  const suffix = `${Date.now()}-${Math.floor(Math.random() * 9999)}`;
  return {
    name: "플로우테스터",
    email: `guard-add-complete-${suffix}@example.com`,
...
/**
 * 회원가입 헬퍼: /signup → 폼 제출 → /dashboard → 로그아웃으로
 * 테스트 계정을 만들고 깨끗한 비인증 상태로 복귀한다.
 */
  ...
  () => {
    ...
    test(
      ...
      async ({ page }) => {
        ...
        // Assert: 인사말에 사용자 이름이 표시된다
        await expect(page.getByTestId("greeting")).toContainText(
          `안녕하세요, ${user.name}님`
        );

        // Assert: 초기 통계가 모두 0이다
        expect(await readStat(page, "stat-total")).toBe(0);
        expect(await readStat(page, "stat-completed")).toBe(0);
        expect(await readStat(page, "stat-pending")).toBe(0);

        // Step 2: 대시보드 빠른 추가 폼으로 할 일 추가
        const todoText = "로그인-추가-완료-통계 전체 플로우 회귀";
        await page.getByTestId("todo-input").fill(todoText);
        await page.getByTestId("todo-add").click();

        // Assert: 할 일 추가 직후 통계 즉시 반영 (전체+1, 진행중+1, 완료 0 유지)
        await expect(
          page.getByTestId("stat-total").locator("p").first()
        ).toHaveText("1");
        await expect(
          page.getByTestId("stat-completed").locator("p").first()
        ).toHaveText("0");
        await expect(
          page.getByTestId("stat-pending").locator("p").first()
        ).toHaveText("1");
        ...
        // Assert: /todos에서 방금 추가한 할 일이 표시된다
    ...
    // -------------------------------------------------------------------
    // 2. 상태 지속성: /todos → /dashboard 이동 및 새로고침 후에도 통계가 유지된다
    // -------------------------------------------------------------------
    test(
      ...
      async ({ page }) => {
        ...
        // Arrange: 로그인 → 할 일 추가 → /todos에서 완료 처리
        await page.goto("/login");
        await page.getByTestId("login-email").fill(user.email);
        await page.getByTestId("login-password").fill(user.password);
        await page.getByTestId("login-submit").click();
    ...
    // -------------------------------------------------------------------
    // 3. 상태 지속성: /dashboard → /todos 경로 전환 및 새로고침 후에도 인증 상태가 유지된다
    // -------------------------------------------------------------------
    test(
      ...
      async ({ page }) => {
        ...
        // Assert: 대시보드에서 인증 상태 확인
        await expect(page.getByTestId("greeting")).toContainText(user.name);
        await expect(page.getByTestId("nav-user-email")).toContainText(
          user.email
        );
    ...
    test(
      ...
      async ({ page }) => {
        ...
        await expect(
          page.getByTestId("stat-completed").locator("p").first()
        ).toHaveText(String(completedBefore));
    ...
    // -------------------------------------------------------------------
    // 6. 상태 지속성 (세션 종료): 로그아웃 시 localStorage 인증 키가 완전히 삭제된다
