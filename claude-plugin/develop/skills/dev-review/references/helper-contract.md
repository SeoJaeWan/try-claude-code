...
The deterministic generator is `claude-plugin/develop/skills/dev-review/scripts/generate-review-data.mjs`. The skill invokes it once per round and relies on its output without re-validating git state.
...
4. **Available agents** (`--available-agents-dir`, repeatable)
   - Defaults (in order): `${workspaceRoot}/claude-plugin/develop/agents/*.md`, `${workspaceRoot}/.claude/agents/*.md`, `${CLAUDE_PLUGIN_ROOT}/develop/agents/*.md`, `${CLAUDE_PLUGIN_ROOT}/agents/*.md`.
