...
test("frontend --help --full은 전체 command contract audit용 JSON을 반환한다", () => {
  ...
  assert.equal(
    ...
    "page.tsx/index.tsx for React page/components, index.ts for hooks"
  );
...
test("frontend --help --full은 fieldResolver와 validateFile target rule도 노출한다", () => {
  const result = runCli(frontendBin, ["--help", "--full"]);
  ...
  assert.equal(payload.commands.validateFile.contracts.inputShape.directoryField, "<path>");
  assert.equal(payload.commands.validateFile.targetRules.at(-1).commandName, "apiHook");
});
