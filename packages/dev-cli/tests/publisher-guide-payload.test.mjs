...
test("publisher guide model은 profile guide와 command contract를 한 구조로 조립한다", async () => {
  ...
  const validateFileSection = model.sections.find((section) => section.id === "validateFile");
  assert.equal(validateFileSection.groups.some((group) => group.id === "targets"), true);
});
...
test("optional group가 비어 있으면 빈 placeholder 대신 해당 group을 생략한다", () => {
  ...
  assert.equal(batchSection.groups.some((group) => group.id === "targets"), false);
