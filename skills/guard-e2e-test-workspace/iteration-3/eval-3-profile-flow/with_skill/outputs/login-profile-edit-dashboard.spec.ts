...
test.describe("[Guard] 로그인 → 프로필 수정 (이름·스킬) → 저장 → 대시보드 인사말 반영 → 프로필 재진입 데이터 유지 여정", () => {
  test("[JOURNEY-PROFILE-001] 이름·스킬 수정 저장 후 대시보드 인사말에 새 이름이 즉시 반영된다", async ({
    page,
  }) => {
  ...
  test("[JOURNEY-PROFILE-001] 프로필 수정 저장 후 대시보드 경유 재진입 시 이름·스킬이 유지된다 (상태 유지 검증)", async ({
    page,
  }) => {
  ...
  test("[JOURNEY-PROFILE-001] 페이지 새로고침 후 수정된 프로필 데이터가 localStorage에서 복원된다", async ({
    page,
  }) => {
  ...
  test("[JOURNEY-PROFILE-001] 프로필 수정 취소 시 이름·스킬이 변경되지 않고 대시보드 인사말도 그대로다 (데이터 불변 분기)", async ({
    page,
  }) => {
  ...
  test("[JOURNEY-PROFILE-001] 비인증 상태에서 /profile 직접 접근 시 /login으로 리다이렉트된다", async ({
    page,
  }) => {
  ...
  test("[JOURNEY-PROFILE-001] 이름 비워두고 저장 시 에러가 표시되고 데이터·인사말이 변경되지 않는다", async ({
    page,
  }) => {
