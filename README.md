...
| `$workbench:execute-task` | Sol/high 조정자 권장, task별 Luna/xhigh 서브에이전트와 전용 Git worktree로 실행 |
...
Shape와 Prepare는 GPT-6 Astra 또는 GPT-5.6 Sol, Execute Task의 메인 조정자는 GPT-5.6 Sol/high를 권장합니다. 이 모델은 스킬을 호출하는 작업에서 선택하며 스킬이 현재 작업의 모델을 자동으로 바꾸지는 않습니다. Execute Task가 생성하는 구현·통합 worker는 GPT-5.6 Luna/xhigh로 고정합니다.
...
- Execute Task의 coordinator는 읽기 전용이며, 각 task를 `gpt-5.6-luna`/`xhigh` worker에게 배정합니다.
