...
| `$workbench:execute-task` | bounded objective 또는 packet 하나를 전용 Git worktree에서 실행 |
| `$workbench:memory-update` | MCP가 지원하는 완료 artifact 하나를 선택 저장 |
| `$workbench:finalize` | 정확한 immutable Git 변경을 위험 기반으로 검증하고 독립 리뷰 |
...
- 입력의 producer보다 완전성, repository identity, digest와 immutable Git SHA를 검증합니다.
- Shape와 Prepare는 현재 checkout을 읽기 전용으로 사용합니다.
- Execute Task만 전용 standard Git worktree에서 변경하고 성공 시 task-local commit을 만듭니다.
...
Workbench는 Local Work Memory MCP에서 프로젝트 Convention과 canonical 문서를 읽고, 사용자가 제공한 Artifact reference를 해소하며, 명시적으로 선택한 완료 artifact를 저장합니다. Artifact kind, folder, transfer, idempotency와 Typed Reference는 MCP tool contract가 소유합니다.
...
`npm test`는 manifest, explicit invocation, 스킬 간 참조 부재, producer-neutral 입력, Git 격리 계약과 배포 안전장치를 검증합니다. `.codex/skills/evaluate-workbench/`는 legacy v2 evaluator이므로 현재 다섯 스킬의 품질 증거가 아닙니다.
