...
export default function DashboardLayout({
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-zinc-950">
      {/* Desktop Sidebar */}
      <aside
        className="hidden md:flex md:w-60 lg:w-64 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 fixed inset-y-0 left-0 z-20"
      ...
      >
        ...
        <div className="flex h-14 shrink-0 items-center border-b border-zinc-200 px-5 dark:border-zinc-800">
          <Link
            href="/"
            className="text-base font-bold text-zinc-900 dark:text-zinc-50 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                data-testid={`sidebar-nav-${link.href.replace(/\//g, "")}`}
              >
                {link.icon}
                {link.label}
              </Link>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900 transition-transform duration-300 md:hidden ${
      ...
      <div className="flex flex-1 flex-col min-w-0 md:pl-60 lg:pl-64">
        {/* Header */}
        <header
          className="sticky top-0 z-10 flex h-14 shrink-0 items-center justify-between border-b border-zinc-200 bg-white px-4 md:px-6 dark:border-zinc-800 dark:bg-zinc-900"
        ...
        >
          <div className="flex items-center gap-3">
            ...
            {/* Desktop page title */}
            <h1
              className="text-base font-semibold text-zinc-900 dark:text-zinc-50 hidden md:block"
          ...
          <div className="flex items-center gap-2" data-testid="dashboard-header-right">
            {headerRight ?? (
              <>
                ...
                <button
                  type="button"
                  className="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-zinc-800 transition-colors"
                  aria-label="알림"
                ...
                <button
                  type="button"
                  className="h-8 w-8 rounded-full bg-zinc-200 dark:bg-zinc-700 hover:ring-2 hover:ring-blue-500 hover:ring-offset-1 transition-all"
                  aria-label="사용자 메뉴"
