...
- Do NOT copy `index.html` or `assets/vendor/` into the data folder. The server now serves those from the plugin html-root (see Step 6 server step). The data folder is data-only — keeping it free of UI assets means a UI bug fix in the plugin propagates to every prior task without re-running anything.
