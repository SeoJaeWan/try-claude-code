...
Three JSON artifacts live under `plans/{key}/dev-review/`, where `key` is the plan directory's relative path under `plans/` (so `foo/bar` for a nested plan at `plans/foo/bar.plan.md`, or `foo` for a folder-style plan at `plans/foo/plan.md`). They share a `schema_version` of `2` and are written with 2-space indentation.
