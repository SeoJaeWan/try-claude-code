...
### ReviewCard 중복
- `components/ReviewCard.tsx` — flat 파일 방식, `export default function` 문법
- `components/common/reviewCard/index.tsx` — folder/index 방식, `const` + `export default` 문법
- 두 파일은 동일한 props(`ReviewCardProps`)와 동일한 시각 결과물을 가짐
- `app/showcase/page.tsx`는 `@/components/common/reviewCard`만 import하고 있었음 (flat 파일은 어디서도 import되지 않음)
...
### 1. 중복 제거
- `components/ReviewCard.tsx` 삭제 (미사용 중복 구현)
