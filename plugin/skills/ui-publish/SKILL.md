...
The very first thing you do — before reading existing code, before writing any file — is scaffold every new component through `tcp`. Run `tcp --help` first to see available commands, path/naming policies, and examples. If placement is unclear, run `tcp guide component` and follow the `components/common/{component}` or `components/{domain}/{component}` rule.
...
4. **Run `tcp --help`, then `tcp guide component` when placement is ambiguous, then `tcp component --apply` (or `tcp batch --apply`) for every new component** — this is the first action before writing any code
