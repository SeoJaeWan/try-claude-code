      ...
      {!editing ? (
        <div className="mt-6 space-y-4" data-testid="profile-view">
          <div>
            ...
            <p className="text-zinc-900 dark:text-zinc-50" data-testid="profile-display-name">{draft.name || "-"}</p>
          </div>
          <div>
            <span className="text-sm text-zinc-500">자기소개</span>
            <p className="text-zinc-900 dark:text-zinc-50">{draft.bio || "-"}</p>
          </div>
          <div>
            <span className="text-sm text-zinc-500">기술 스택</span>
            <p className="text-zinc-900 dark:text-zinc-50">{draft.skills.join(", ") || "-"}</p>
          </div>
          <div>
            <span className="text-sm text-zinc-500">생년월일</span>
            <p className="text-zinc-900 dark:text-zinc-50">{draft.birthDate || "-"}</p>
          </div>
          <div>
            <span className="text-sm text-zinc-500">아바타</span>
            <p className="text-zinc-900 dark:text-zinc-50">{draft.avatarFileName || "-"}</p>
          </div>
          <button
            onClick={handleEdit}
        ...
        <div className="mt-6 space-y-4" data-testid="profile-form">
          ...
          <DatePicker label="생년월일" value={draft.birthDate} onChange={handleBirthDateChange} testId="profile-birthdate" />
          ...
          <div>
            ...
            <FileUpload value={draft.avatarFileName} onChange={handleAvatarChange} />
          </div>
